...
import { GitHubLinkChip, GitHubLinkProject } from './GitHubLinkChip'
...
describe('GitHubLinkChip', () => {
  ...
  it('opens a menu whose actions reach the canvas handler', () => {
    ...
    setGitHubLinkHandler({ attach: vi.fn(), detach, set: vi.fn(), openPicker, openDetails: vi.fn() })
    ...
    expect(detach).toHaveBeenCalledWith('n1', { kind: 'pull', number: 3, title: 'Ship' }, undefined)
  })

  it('opens the item on github.com through the shell, at its own kind’s path', () => {
    setGitHubLinkHandler({ attach: vi.fn(), detach: vi.fn(), set: vi.fn(), openPicker: vi.fn(), openDetails: vi.fn() })
  ...
  it('inside an Omni lane, resolves and acts against the lane’s project, not the active one', () => {
    useProjects.setState({
      activeProjectId: 'p1',
      projects: [
        { id: 'p1', name: 'A', color: '#fff', viewport: { x: 0, y: 0, zoom: 1 }, nodes: [] },
        {
          id: 'p2', name: 'B', color: '#fff', viewport: { x: 0, y: 0, zoom: 1 }, nodes: [],
          kanban: { columns: [], assignments: [], github: { repository: 'o/other', columnMappings: [] } }
        }
      ]
    } as never)
    useGitHubLinks.setState({ cards: { p2: { 'issue#12': { card: card(12, { state: 'closed' }), at: Date.now() } } } })
    const detach = vi.fn()
    setGitHubLinkHandler({ attach: vi.fn(), detach, set: vi.fn(), openPicker: vi.fn(), openDetails: vi.fn() })
    act(() => root.render(
      <GitHubLinkProject.Provider value="p2">
        <GitHubLinkChip nodeId="n1" links={[{ kind: 'issue', number: 12 }]} variant="card" />
      </GitHubLinkProject.Provider>
    ))
    // p1 has no repository, so an unscoped chip would render nothing at all.
    expect(host.querySelector('.github-link-chip__dot')?.className).toContain('--closed')
    act(() => {
      host.querySelector('.github-link-chip')!
        .dispatchEvent(new MouseEvent('click', { bubbles: true }))
    ...
    const row = [...document.querySelectorAll('.ctx-menu button')]
      .find((b) => b.textContent === 'Detach #12')!
    act(() => row.dispatchEvent(new MouseEvent('click', { bubbles: true })))
    expect(detach).toHaveBeenCalledWith('n1', { kind: 'issue', number: 12 }, 'p2')
  })
})
